...
bool Query_Parser::requireDbOpen() {
    if (!current->isOpen()) {
...
void Query_Parser::printReadableException(int err) {
    printf("Exception: ");
    printf("%s\n", Exception2String[err]);
}

void Query_Parser::printExprVal(const Expression &val) {
    switch (val.type) {
        case TERM_INT:
...
bool Query_Parser::convertToBool(const Expression &val) {
    bool t = false;
    switch (val.type) {
        case TERM_INT:
...
Expression Query_Parser::dbTypeToExprType(char *data, ColumnType type) {
    Expression v;
...
term_type Query_Parser::ColumnTypeToExprType(const ColumnType &type) {
    switch (type) {
        case CT_INT:
            return TERM_INT;
        case CT_FLOAT:
...
bool Query_Parser::checkColumnType(ColumnType type, const Expression &val) {
...
char *Query_Parser::ExprTypeToDbType(Expression &val, term_type desiredType) {
    char *ret = nullptr;
...
void Query_Parser::cacheColumns(Table *tb, int rid) {
...
void Query_Parser::freeCachedColumns() {
...
Query_Parser::IDX_TYPE Query_Parser::checkIndexAvailability(Table *tb, RID_t *rid_l, RID_t *rid_u, int *col, expr_node *condition) {
...
RID_t Query_Parser::nextWithIndex(Table *tb, IDX_TYPE type, int col, RID_t rid, RID_t rid_u) {
    if (type == IDX_EQUAL) {
...
void Query_Parser::iterateRecords(linked_list *tables, expr_node *condition, CallbackFunc callback) {
...
expr_node *Query_Parser::findJoinCondition(expr_node *condition) {
...
bool
Query_Parser::iterateTwoTableRecords(Table *a, Table *b, expr_node *condition, CallbackFunc callback) {
...
void Query_Parser::iterateRecords(Table *tb, expr_node *condition, CallbackFunc callback) {
...
int Query_Parser::isAggregate(const linked_list *column_expr) {
    int flags = 0;
...
void Query_Parser::freeLinkedList(linked_list *t) {
    linked_list *next;
    for (; t; t = next) {
        next = t->next;
        free(t);
...
Query_Parser *Query_Parser::getInstance() {
    static Query_Parser *instance;
    if (!instance)
        instance = new Query_Parser;
    return instance;
}

void Query_Parser::exit() {
    if (current->isOpen())
        current->close();
}

void Query_Parser::switchToDB(const char *name) {
    if (current->isOpen())
        current->close();
...
void Query_Parser::createTable(const table_def *table) {
    if (!requireDbOpen())
        return;
...
void Query_Parser::dropDB(const char *db_name) {
    Database db;
...
void Query_Parser::dropTable(const char *table) {
    if (!requireDbOpen())
        return;
...
void Query_Parser::selectRow(const linked_list *tables, const linked_list *column_expr, expr_node *condition) {
    int flags;
    if (!requireDbOpen())
        return;
...
void Query_Parser::updateRow(const char *table, expr_node *condition, column_ref *column, expr_node *eval) {
    Table *tb;
    if (!requireDbOpen())
        return;
    if (!(tb = current->getTableByName(table))) {
...
void Query_Parser::deleteRow(const char *table, expr_node *condition) {
...
void Query_Parser::insertRow(const char *table, const linked_list *columns, const linked_list *values) {
    Table *tb;
    if (!requireDbOpen())
        return;
    if (!(tb = current->getTableByName(table))) {
...
void Query_Parser::createIndex(column_ref *tb_col) {
    Table *tb;
    if (!requireDbOpen())
        return;
...
void Query_Parser::dropIndex(column_ref *tb_col) {
    Table *tb;
    if (!requireDbOpen())
        return;
    if (!(tb = current->getTableByName(tb_col->table))) {
...
void Query_Parser::descTable(const char *name) {
    Table *tb;
    if (!requireDbOpen())
        return;
...
bool Query_Parser::valueExistInTable(const char *value, const ForeignKey &key) {
